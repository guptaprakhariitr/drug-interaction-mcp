...
### Fixed
- `check_interactions_multi` returned the same pair twice (A-B and B-A). Now deduplicates by canonical pair ordering.
- Severity normalization: NLM's interaction severity vocabulary differs from clinical convention; added a translation table.

## [0.1.0] — 2026-05-15

### Added
- Initial release. Tools: `check_interaction`, `check_interactions_multi`, `normalize_drug_name`, `dose_guide`, `find_alternatives`.
- Wraps RxNorm + DailyMed.
- Every response includes source citations (RxNorm relationships + DailyMed label sections).
